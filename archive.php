...
window.onload = function(){ //避免爆代码
        ...
        new Vue({ //axios获取顶部信息
            ...
            mounted () {
                ...
                //获取文章列表
                axios.get('<?php echo site_url() ?>/wp-json/wp/v2/posts?per_page=10&page='+paged+'&categories='+incate)
                 .then(response => {
...
　　                    }
            ...
            filters: {
                link_page : function(cate_id){
                    if(cate_id == <?php echo get_option('king_fre_cate'); ?>){
                        return '添加于 ';
                    }else if(cate_id == <?php echo get_option('king_wor_cate'); ?>){
                        return '创造于 ';
                    }else{
                        return '';
                    }
                ...
                link_style : function(cate_id){
                    if(cate_id == <?php echo get_option('king_fre_cate'); ?> || cate_id == <?php echo get_option('king_wor_cate'); ?>){
                        return 'display: flex;';
                    }else{
                        return '';
                    }
